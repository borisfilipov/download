...
def get_synthetics_test_by_id(test_id: str):
    try:
        configuration = get_configuration()
        with ApiClient(configuration) as api_client:
            api_instance = SyntheticsApi(api_client)
            result = api_instance.get_api_test(public_id=test_id)
            print(f"Name: {result.name}")
            print(f"Type: {result.type}, Subtype: {result.subtype}")
            print(f"Request: {result.config.request}")
            print(f"Assertions: {result.config.assertions}")
            return result
    except Exception as e:
        print(f"Failed to fetch test ID {test_id}: {str(e)}")
        return None

def list_ssl_synthetics_tests():
    try:
        configuration = get_configuration()  # Assumes your get_configuration() is valid
        with ApiClient(configuration) as api_client:
            api_instance = SyntheticsApi(api_client)

            tests = api_instance.list_tests().tests
            ssl_tests = [t for t in tests if t.subtype == SyntheticsTestDetailsSubType.SSL]

            print(f"Found {len(ssl_tests)} SSL tests:")
            for test in ssl_tests:
                print(f"- {test.name} (ID: {test.public_id})")

            return ssl_tests

    except Exception as e:
        print(f"Error fetching SSL Synthetics tests: {str(e)}")
        return []
...
def check_missing_checkmarks_old(excel_stream=None):
    try:
...
def check_missing_checkmarks(excel_stream=None):
    try:
        # Read the 'alert' worksheet from Excel file
        if excel_stream:
            df = pd.read_excel(excel_stream, sheet_name='alert', engine='openpyxl', header=0)
        else:
            df = pd.read_excel('DD_alert.xlsx', sheet_name='alert', engine='openpyxl', header=0)

        # Identify how many columns to check based on non-empty header row values
        max_col = 2  # Start from column index 2 (Excel column C)
        while max_col < len(df.columns) and pd.notna(df.iloc[0, max_col]):
            max_col += 1

        # Extract the relevant headers
        headers = df.columns[2:max_col]
        ...
        for row_idx in range(1, min(non_empty_count + 1, len(df))):
            row_label = df.iloc[row_idx, 1]  # Column B
            for col_idx, header in enumerate(headers):
                cell_value = df.iloc[row_idx, col_idx + 2]  # +2 to skip A and B
                if (pd.isna(cell_value) or str(cell_value).strip() == "") and str(cell_value).lower().strip() != 'n/a':
                    missing_checks.append({
                        'header': header,
                        'cell': f"{get_column_letter(col_idx + 3)}{row_idx + 2}",  # Excel notation
                        'row_label': row_label
                    })
...
def get_monitor_name_from_file(config_data):
    try:
            config_df = config_data.get("config")
            if config_df is None:
                print("Error: 'config' sheet not found")
                return

            # Reconstruct column-based dict as in read_config_data_o
            reconstructed_config = {
                col: config_df[col][1:].dropna().tolist()  # skip header row
            ...
            with open('missing_checkmarks.txt', 'r') as f:
                for line in f:
                    if 'Row:' in line and 'Header:' in line:
                        ...
                        matching_array = reconstructed_config.get(header)
                        if not matching_array:
                            print(f"Header '{header}' not found in config data")
                            continue
                        dispatch_map = {
                        ...
                        if func:
                            ...
                            raise ValueError(f"Unsupported monitor type: '{row_value}' (header: {header})")
    except Exception as e:
...
def eks_pod_crashloop(header, matching_array):
    ...
    try:
        ...
        # debugging    
        print(monitor_environment)
        print(kube_clustername)
...
def update_monitor(config_data):
    ...
    try:
        df = config_data.get('update_monitor')
        if df is None:
            print("Error: 'update_monitor' sheet not found in Excel input")
            return
...
def delete_monitor(config_data):
    ...
    try:
        df = config_data.get('delete_monitor')
        if df is None:
            print("Error: 'delete_monitor' sheet not found in Excel input")
            return
...
def create_monitor(config_data):
    ...
    try:
        df = config_data.get('create_monitor')
        if df is None:
            print("Error: 'create_monitor' sheet not found in Excel input")
            return
...
def create_synthetics(config_data):
    """Creates Datadog Synthetics tests from config_data in 'synthetics' tab."""
    try:
        df = config_data.get('create_synthetics')
        if df is None:
            print("Error: 'create_synthetics' sheet not found in Excel input")
            return
        ...
        with ApiClient(configuration) as api_client:
            ...
            for _, row in df.iterrows():
                if pd.isna(row['name']) or (pd.isna(row['request_url']) and pd.isna(row.get('host'))):
                    print(f"Skipping invalid row: {row}")
                    continue

                if pd.isna(row.get('subtype')):
                    raise ValueError(f"Missing subtype for test '{row['name']}'")

                subtype_str = str(row['subtype']).upper()
                if subtype_str not in subtype_map:
                    raise ValueError(f"Unsupported subtype '{subtype_str}' for test '{row['name']}'")

                subtype = subtype_map[subtype_str]

                assertions = []
                if 'assertions_json' in row and pd.notna(row['assertions_json']):
                    try:
                        assertions_raw = json.loads(row['assertions_json'])
                        print(assertions_raw)
                        assertions = [
                            SyntheticsAssertionTarget(
                                operator=SyntheticsAssertionOperator(item['operator']),
                                type=SyntheticsAssertionType(item['type']),
                                target=item['target']
                            ) for item in assertions_raw
                        ]
                    except Exception as ex:
                        raise ValueError(f"Invalid JSON in assertions_json for test '{row['name']}': {ex}")

                options_dict = {
                    "tick_every": int(row.get('frequency', 300)),
                    "retry": {"count": 2, "interval": 10},
                }
                if 'priority' in row and pd.notna(row['priority']):
                    options_dict['monitor_priority'] = int(row['priority'])

                test_options = SyntheticsTestOptions(**options_dict)
                tags = row['tags'].split(',') if 'tags' in row and pd.notna(row['tags']) else []
                ...
                if subtype == "ssl":
                    config = SyntheticsAPITestConfig(
                        ...
                        #assertions=[SyntheticsAssertion(operator="isInMoreThan", type="certificate", target=30),]
                        assertions=assertions
                    )
                    print(assertions)
                    test_body = SyntheticsAPITest(
                        config=config,
                        locations=row['locations'].split(',') if pd.notna(row.get('locations')) else ["aws:us-east-2"],
                    ...
                    method = str(row.get('request_method', 'GET')) if pd.notna(row.get('request_method')) else 'GET'
                ...
                result = api_instance.create_synthetics_api_test(body=test_body)
                print(f"Synthetics test created successfully with Public ID: {result.public_id}")

    except Exception as e:
        print(f"Error processing synthetics create: {str(e)}")

def delete_synthetics(config_data):
    """Deletes Datadog Synthetics tests using config_data from 'delete' tab."""
    try:
        df = config_data.get('delete_synthetics')
        if df is None:
            print("Error: 'delete_synthetics' sheet not found in Excel input")
            return
        ...
        with ApiClient(configuration) as api_client:
            ...
            for _, row in df.iterrows():
                ...
                tests = api_instance.list_tests().tests
                matched = next((t for t in tests if t.name == test_name), None)

                if not matched:
                    print(f"Synthetics test '{test_name}' not found for deletion.")
                    continue
                payload = SyntheticsDeleteTestsPayload(public_ids=[matched.public_id])
                api_instance.delete_tests(body=payload)
                print(f"Deleted synthetics test '{test_name}' with ID: {matched.public_id}")

    except Exception as e:
        print(f"Error processing delete data: {str(e)}")

def update_synthetics(config_data):
    """Updates Datadog Synthetics tests using config_data from 'synthetics' tab."""
    try:
        df = config_data.get('update_synthetics')
        if df is None:
            print("Error: 'update_synthetics' sheet not found in Excel input")
            return
        ...
        with ApiClient(configuration) as api_client:
            api_instance = SyntheticsApi(api_client)
            existing_tests = api_instance.list_tests().tests

            for _, row in df.iterrows():
                test_name = row.get("name")
                matched = next((t for t in existing_tests if t.name == test_name), None)

                if not matched:
                    print(f"Synthetics test '{test_name}' not found for update.")
                    continue

                subtype = subtype_map.get(str(row.get('subtype')).upper(), None)
                if not subtype:
                    print(f"Unsupported or missing subtype for test '{test_name}'")
                    continue
                ...
                if 'assertions_json' in row and pd.notna(row['assertions_json']):
                    try:
                        assertions_raw = json.loads(row['assertions_json'])
                        for item in assertions_raw:
                            assertions.append(SyntheticsAssertionTarget(
                                operator=SyntheticsAssertionOperator(item['operator']),
                                type=SyntheticsAssertionType(item['type']),
                                target=item['target']
                            ))
                    except Exception as ex:
                        raise ValueError(f"Invalid JSON in assertions_json for test '{test_name}': {ex}")
                ...
                if subtype == "ssl":
                    ...
                    result = api_instance.update_api_test(public_id=matched.public_id, body=updated_test)
                else:
                    request_method = str(row.get('request_method', 'GET'))
                    test_request = SyntheticsTestRequest(
                        url=row['request_url'],
                ...
                print(f"Updated synthetics test '{test_name}' (ID: {matched.public_id})")
...
def get_configuration():
    ...
    configuration.api_key["apiKeyAuth"] = "50a8d938e5c0aff315e44df1365c4ca5"
    configuration.api_key["appKeyAuth"] = "b0e07d3b09a7d37352c2dfbd43632c205d45daf7"
    
    #api_key = "abdb48a154d39f6c43ba384d347526ff"
    #app_key = "361cfe152f34c85564db7f1de0af881f8d963243"
    #papi_key = "50a8d938e5c0aff315e44df1365c4ca5"
    #papp_key = "b0e07d3b09a7d37352c2dfbd43632c205d45daf7"
...
def handle_command(command, config_data, buffer=None):
    ...
    elif command == "createmonitor":
        create_monitor(config_data)
    elif command == "updatemonitor":
        update_monitor(config_data)
    elif command == "deletemonitor":
        delete_monitor(config_data)
    elif command == "createsynthetics":
        create_synthetics(config_data)
    elif command == "deletesynthetics":
        delete_synthetics(config_data)
    elif command == "updatesynthetics":
        update_synthetics(config_data)
    else:
        print(f"Command '{command}' is not yet implemented.")  
      
if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--command", type=str, help="Command to execute: validate, createmonitor, deletemonitor, updatemonitor, createsynthetics, deletesynthetics, updatesynthetics", required=True)
    parser.add_argument("--id", type=str, help="Parameter Monitor ID", required=False)
    ...
    if command == "validate":
        validate_connection()
    elif command == "listssl":
        list_ssl_synthetics_tests()
    elif command == "getssl":
        get_synthetics_test_by_id(monitor_id)
    elif command in {"check", "createmonitor", "updatemonitor", "deletemonitor", "createsynthetics", "deletesynthetics", "updatesynthetics"}:
