...
def login_to_datadog(app_key=None):
    """
...
def eks_pod_crashloop(header, matching_array):
    ...
    try:
        ...
        monitor = {
            ...
            'type': 'query alert',
            'query': f'max(last_5m):default_zero(max:kubernetes_state.container.status_report.count.waiting{{reason:crashloopbackoff ,kube_cluster_name:{kube_clustername}, kube_namespace:{kube_namespace}}} by {{kube_cluster_name,kube_namespace,pod_name}}) >= 1',
